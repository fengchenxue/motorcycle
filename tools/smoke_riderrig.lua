...
断言组:
  ...
  ⑥ 挥砍(ADR-50c Transform 通道)与拖剑火花:贴地高速=喷/挥砍期=熄/离地=熄;挥完 Transform 归 identity
...
local PIN = { Combat_InputBufferSec = 0.25, VFX_SlashSweepSec = 0.12, VFX_SwordDragSparkRate = 40 }
local saved = {}
...
local ctrlTele = { grounded = true, speed = 100 }
local ctrlStub = {}
...
-- ⑥ 挥砍(Transform 通道)+拖剑火花(窗 0.25;起手 0.06+扫掠 0.12+回位 0.25)
do
	...
	swingFn()
	rr:step(DT)
	ok("⑥c 挥砍期肩部 Transform 离开 identity", not nearCF(rShoulder.Transform, CFrame.identity), "未动")
	ok("⑥d 挥砍期火花熄(剑已举起)", spark.Rate == 0, spark.Rate)
	for _ = 2, 35 do rr:step(DT) end               -- 0.58s > 起手0.06+窗0.25+回位0.25
	ok("⑥e 挥完 Transform 归 identity 且状态闲", nearCF(rShoulder.Transform, CFrame.identity) and rr.swingT < 0,
		tostring(rr.swingT))
