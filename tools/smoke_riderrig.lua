...
断言组:
  ...
  ⑥ 挥砍(C0 通道,坑 35)+剑特效(ADR-50d):火花三态/刀光挥砍期开/锋全亮/心流变金退青;挥完 C0 归骑姿
...
local PIN = { Combat_InputBufferSec = 0.25, VFX_SlashSweepSec = 0.12, VFX_SwordDragSparkRate = 40, VFX_SwordTrailSec = 0.25 }
local saved = {}
...
-- ⑥ 挥砍(C0 通道,坑 35)+剑特效(ADR-50d)(窗 0.25;起手 0.06+扫掠 0.12+回位 0.25)
do
	local spark = sword.Edge.SwordTip.DragSpark
	local trail = sword.Edge.SwordTrail
	local edge = sword.Edge
	rr:step(DT)
	ok("⑥a 拖剑火花:贴地高速=开(Rate=40)", spark.Rate == 40, spark.Rate)
	ok("⑥b swing 事件已被订阅", swingFn ~= nil, "未订阅")
	local idleC0 = rShoulder.C0                     -- 骑姿基准(挥完应回这里;坑 35:C0 通道)
	swingFn()
	rr:step(DT)
	ok("⑥c 挥砍期肩 C0 离开骑姿(C0 通道)", not nearCF(rShoulder.C0, idleC0), "C0 未动")
	ok("⑥d 挥砍期火花熄+刀光开+锋全亮", spark.Rate == 0 and trail.Enabled == true and edge.Transparency == 0,
		spark.Rate .. "/" .. tostring(trail.Enabled))
	for _ = 2, 35 do rr:step(DT) end               -- 0.58s > 起手0.06+窗0.25+回位0.25
	ok("⑥e 挥完 C0 精确归骑姿且闲+刀光关", nearCF(rShoulder.C0, idleC0) and rr.swingT < 0 and trail.Enabled == false,
		tostring(rr.swingT) .. "/" .. tostring(trail.Enabled))
	ctrlTele.grounded = false
	rr:step(DT)
	ok("⑥f 离地火花熄", spark.Rate == 0, spark.Rate)
	ctrlTele.grounded = true
	ctrlTele.flowBoost = true
	rr:step(DT)
	ok("⑥g 心流=剑变金", edge.Color == Color3.fromRGB(255, 200, 80), tostring(edge.Color))
	ctrlTele.flowBoost = false
	rr:step(DT)
	ok("⑥h 退心流=剑回青", edge.Color == Color3.fromRGB(90, 220, 255), tostring(edge.Color))
end
