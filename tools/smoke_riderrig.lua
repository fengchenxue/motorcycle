...
断言组:
  ...
  ⑥ 挥砍(C0 通道,坑 35)+剑特效(ADR-50e):火星/刀光已删断言;锋挥砍全亮;心流金退青;挥完 C0 归骑姿
...
do
	...
	ok("⑥a 火星与刀光已删(无 DragSpark/SwordTrail)", edge:FindFirstChild("DragSpark") == nil and edge:FindFirstChild("SwordTrail") == nil, "残留")
	ok("⑥b swing 事件已被订阅", swingFn ~= nil, "未订阅")
	local idleC0 = rShoulder.C0                     -- 骑姿基准(挥完应回这里;坑 35:C0 通道)
	swingFn()
	rr:step(DT)
	ok("⑥c 挥砍期肩 C0 离开骑姿+锋全亮", not nearCF(rShoulder.C0, idleC0) and edge.Transparency == 0, tostring(edge.Transparency))
	for _ = 2, 35 do rr:step(DT) end               -- 0.58s > 起手0.06+窗0.25+回位0.25
	ok("⑥d 挥完 C0 精确归骑姿且闲", nearCF(rShoulder.C0, idleC0) and rr.swingT < 0, tostring(rr.swingT))
