...
- **更新:** 2026-07-14,拍板清仓会话(宪法升 **v1.1**):**ADR-38** 射手只打前方定稿(背后弹否决)/**ADR-39** 移动端输入分层(默认两拇指+冲刺松键宽限 0.12s AB,实现并入 M6.5 第 8 条;M10 义务=键位自定义+"爪握预设"=食指斩击,人类提案)/§7 节拍配比初版模板/名称沿用 NEON RUN 至发布前。零代码改动。此前同日:文档重构 v1.0(c26f2f8,重构前全文存档 d19ef25)、M8.1 代码落地待验收、ADR-36/37(贴墙段 B+能量两轨)。
...
- **待拍板(人类):** ① 手感参数统一调(`Combat_*`/心流 AB 旋钮属性面板实时生效);④ M6.5 手感三项:进入宽容度/高度带速率与"下坠漂移"开关(默认关)/相机滚转速度;⑤ **最小点火×心流尾段**(现默认=延续,备选=尾段豁免;M9.5 AB,旋钮 `MinIgnitionBurnSec`)。**已拍板归档(2026-07-14):** 贴墙=B(ADR-36)/能量两轨(ADR-37)/射手只打前方(ADR-38)/输入分层+松键宽限(ADR-39)/节拍模板初版(§7)/名称沿用 NEON RUN。
