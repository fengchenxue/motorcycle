...
- **代码同步:** repo 23 脚本(Modules 18+Config 5)+TestDrive = Studio 哈希逐字节一致(2026-07-15 全量 djb2 比对;第三/五会话 BikeController 折线行走+ADR-45 解析圆角+ADR-45a C¹ 入墙混合,复核 len 42174/djb2 675233116 逐字节一致;**第六会话(手感包 v2)Handling/CameraRig/TunePanel 三件 `.Source` 整体推平,djb2 后验=repo:Handling 9307/278194075、CameraRig 10008/1562730560、TunePanel 5835/3539680016**)。**Rojo 插件断连中:插件版本≠服务端 7.7.0(`Can't parse JSON` 刷屏)——人工更新 Studio 端插件到 7.7.0,只留一份插件(坑 25)。**
...
- **Handling Attributes:实测 119**(含 VFX_ 8 键+ADR-44 Camera_ 12 键(v2 增 SteerRollLerpSec/LatLagSec,默认全关)+**ADR-45 `WallRide_FilletRadius=7`**;人类调参值 `WallRide_BlendSec/CamRollSec=0.4` 保留);Energy 17;Config 含 TrackSpecs(ADR-41 声明表)。
