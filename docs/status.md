...
- **Handling Attributes:实测 119**(含 VFX_ 8 键+ADR-44 Camera_ 12 键+**ADR-45 `WallRide_FilletRadius=7`**;人类调参值 `WallRide_BlendSec/CamRollSec=0.4` 保留)。**⚠️ 现值=hot v3 预设应用态(2026-07-15 二轮反馈后 Edit 期落位,27 键偏离源码默认——刻意为之,勿当漂移"修复";还原=preset stock)**;Energy 17;Config 含 TrackSpecs(ADR-41 声明表)。
