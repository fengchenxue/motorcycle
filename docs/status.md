...
- **表现层(07-15,本线):** 冲刺速度线上线——BikeVFX 新增相机空间双层 VFX:6 条 Beam 放射线(贴图 `rbxassetid://6967022163`,手作参照件 `Workspace.VFX.Beam` 可删;屏心远→屏边近,贴图由远及近慢流)+ VelocityParallel 风粒子掠镜;由 `tele.sprinting` 驱动(与点火音效同源,空能量不触发),透明度淡入 0.2s/淡出 0.25s(Beam 常开,硬切 Enabled 跳贴图相位);参数=Handling 新增 VFX 段 8 键(`VFX_SpeedLine*`,bind 自动种,属性面板实时调);客户端渲染循环 `rig:update` 后新增 `vfx:cameraStep()`(相机写完同帧钉位,零滞后)。Edit 冒烟(dt=1/60)八项全过(淡入淡出/钉相机/粒子开关/清理);纯渲染零物理,直线+坡顶回归不受影响。**待人工试玩拍板:线布局(`BikeVFX.SPEEDLINE_OFFSETS`)/粒子浓度/流速。** 副产物:冒烟在 Edit 触发 bind,Handling 全部模块默认键已持久种上(含 M4.1 +6/M6.5 ChainMaxTurnDeg,均为默认值;seed 工具重跑覆盖为准,selfcheck 实测口径)。
...
- **远程 Studio 通路(2026-07-14 实测):** Studio 跑在局域网 Windows 机 `192.168.110.168`(用户 Ymz);MCP=全局配置的 SSH stdio 桥(这台 Linux → 那台的 Roblox 官方 `mcp.bat`,**SSH 已实测通**;工具出现的前提=那头 Studio 开着且 MCP 插件已连,然后本会话 set_active)。**Rojo:本项目 serve 常驻于 tmux 会话 `ljx-rojo-motorcycle`,监听 `192.168.110.69:34873`**(Studio 端 Rojo 插件填此地址连;3087/9908/34872/8123 被其他工位占用勿撞)。管理:`tmux attach -t ljx-rojo-motorcycle` 看现场,`tmux kill-session -t ljx-rojo-motorcycle` 停;机器重启后重起=`tmux new -d -s ljx-rojo-motorcycle -c ~/users/ljx/motorcycle "exec /home/showhand/users/ljx/bin/rojo-7.7.0 serve default.project.json --address 192.168.110.69 --port 34873"`。**⚠️ 服务端=工作区 bin 的 7.7.0 正式版(与 Studio 插件 7.7.0 配对;坑 25)。**`.Source` 推送只作断连兜底,推前先查。**⚠️ repo 无 .rbxl:place 只存在于 Studio 侧,下次 Studio 会话务必存一份 .rbxl 进 repo(灾难还原点,CLAUDE.md 义务)。**
